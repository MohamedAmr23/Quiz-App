...
import { generateQuestions, GeneratedQuestion } from "@/shared/lib/services/generate.service";
...
export default function GenerateQuestionsPage() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setQuestions([]);
      const result = await generateQuestions({ technology: selectedTech.trim(), level, count });
  ...
  function copyQuestion(q: GeneratedQuestion, index: number) {
    const text = `Q: ${q.title}\n${q.options.map((o, i) => `${String.fromCharCode(65 + i)}. ${o.text}${o.isCorrect ? " ✓" : ""}`).join("\n")}`;
  ...
  function copyAll() {
    const text = questions
      .map((q, i) => `${i + 1}. ${q.title}\n${q.options.map((o, j) => `   ${String.fromCharCode(65 + j)}. ${o.text}${o.isCorrect ? " ✓" : ""}`).join("\n")}`)
      .join("\n\n");
    navigator.clipboard.writeText(text);
    setAllCopied(true);
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-700 leading-relaxed">
                  Questions are generated using <span className="font-medium">openrouter/free</span> — 
                  OpenRouter's auto-router that picks the best available free AI model at the time 
                  of your request. This means you always get a working model without worrying about 
                  rate limits or unavailability.
                </p>
                {usedModel && (
                  <div className="mt-3 flex items-center gap-2 rounded-lg bg-blue-100 px-3 py-2">
                    ...
                    <p className="text-xs text-blue-800">
                      Last used model:{" "}
                      <span className="font-bold">{usedModel}</span>
                    </p>
                  </div>
                )}
              </div>
          ...
          >
            {isGenerating ? (
              <>
                ...
                Generating {count} questions about {selectedTech || "..."}
              </>
            ) : (
              <>
                <Sparkles size={18} />
              ...
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="rounded-xl border border-gray-100 bg-white p-4">
                  ...
                  <div className="space-y-2">
                    {[0, 1, 2, 3].map((j) => (
                      <div key={j} className="h-3 w-full animate-pulse rounded bg-gray-50" />
                    ))}
                  </div>
                </div>
              ))}
              ...
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {selectedTech} · {level} · {questions.length} questions
                  </p>
                </div>
                <button
                  onClick={copyAll}
